...
import { useSchedulesMutations, useSchedule } from "../../../hooks/schedules/useSchedules";
import { daysOfWeek } from "../../../constants";
import type { ScheduleFormData } from "../../../types";

export default function ScheduleUpdateFormContent({ id }: { id: string }) {
    const { updateScheduleMutation } = useSchedulesMutations();
    ...
    useEffect(() => {
        if (!isLoading && schedules && !scheduleToEdit) {
            toast.error("Horario no encontrado");
            navigate("/dashboard/doctor/schedule", { replace: true });
        }
    }, [isLoading, schedules, scheduleToEdit, navigate]);

    const onSubmit = (data: ScheduleFormData) => {
        updateScheduleMutation.mutate({ id, scheduleData: data });
    };
        ...
        <div className="max-w-md mx-auto">
            <form className="space-y-5 bg-white p-6 rounded-xl shadow-sm border border-gray-200" onSubmit={handleSubmit(onSubmit)}>
                ...
                <button
                    ...
                    type="submit"
                    className={`w-full flex justify-center py-3.5 px-4 border border-transparent rounded-xl shadow-sm text-sm font-semibold text-white bg-linear-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 transition-all duration-200 ${(updateScheduleMutation.isPending || Object.keys(errors).length > 0) ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                    Guardar Horario
                </button>
            </form>
