...
import type { ScheduleFormData, Slot } from "../types";

export const getSlots = async (page: number, limit: number, day?: string, date?: string) => {
...
export const updateSlotById = async (id: string, slotData: Slot) => {
    const url = `${import.meta.env.VITE_API_URL}/slots/${id}`;
    const response = await api.put(url, slotData);
    return response.data;
}

export const generateScheduleAndSlots = async (scheduleData: ScheduleFormData) => {
...
export const updateSchedule = async (id: string, scheduleData: ScheduleFormData) => {
